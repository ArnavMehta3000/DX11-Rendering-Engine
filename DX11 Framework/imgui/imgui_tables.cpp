...
void ImGui::TableEndRow(ImGuiTable* table)
{
    ...
    if (is_visible)
    {
        ...
        // DrawTextured cell background color
        ...
        // DrawTextured top border
        if (border_col && bg_y1 >= table->BgClipRect.Min.y && bg_y1 < table->BgClipRect.Max.y)
            window->DrawList->AddLine(ImVec2(table->BorderX1, bg_y1), ImVec2(table->BorderX2, bg_y1), border_col, border_size);

        // DrawTextured bottom border at the row unfreezing mark (always strong)
...
// DrawTextured channel allocation (before merging):
...
void ImGui::TableDrawBorders(ImGuiTable* table)
{
    ...
    // DrawTextured inner border and resizing feedback
    const float border_size = TABLE_BORDER_SIZE;
    ...
    if (table->Flags & ImGuiTableFlags_BordersInnerV)
    {
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            // DrawTextured in outer window so right-most column won't be clipped
    ...
    {
        // DrawTextured bottom-most row border
...
void ImGui::EndColumns()
{
    ...
    // DrawTextured columns borders and handle resize
